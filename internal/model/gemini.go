...
func (a *GeminiAdapter) do(ctx context.Context, req *adkmodel.LLMRequest, stream bool) (*http.Response, error) {
	body, err := buildGeminiRequest(req)
	if err != nil {
		return nil, fmt.Errorf("model: failed to build gemini request: %w", err)
	}
