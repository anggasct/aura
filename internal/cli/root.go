...
func newRootCmd() *cobra.Command {
	...
	root.AddCommand(newVersionCmd(), newServerCmd(gf), newChatCmd(gf), newExecCmd(), newStorageCmd(gf), newUsageCmd(gf), newEffectsCmd(gf), newAgentsCmd(gf), newSkillsCmd(gf), newProfileCmd(gf), newChildrenCmd(gf), newWorkflowCmd(gf), newDurableCmd(gf), newBroadcastCmd(gf), newScheduleCmd(gf), newStatusCmd(gf, sandbox.Negotiate), newDoctorCmd(gf, sandbox.Negotiate), newModelsCmd(gf), newSyncCmd(gf))
	return root
}
